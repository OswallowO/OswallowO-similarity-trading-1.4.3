                ...
                try:
                    with open('auto_intraday.json', 'r', encoding='utf-8') as f:
                        ...
                        if symbol in existing_data and len(existing_data[symbol]) > 0:
                            previous_close = existing_data[symbol][-1]['close']
                except Exception as e:
...
def start_trading():
        ...
        if not daily_data:
            ...
            if not daily_kline_df.empty:
                ...
                if len(daily_kline_data) > 1:
                    yesterday_close = daily_kline_data[1].get('close', 0)
    ...
    market_end = current_time.replace(hour=23, minute=59, second=0, microsecond=0)
...
#盤中交易
group_observation = {}  # 格式：{ group: {"trigger": str, "start_time": datetime, "tracking": {}, "leader": None, "threshold": None, "waiting": False, "waiting_time": 0} }

def get_group_for_stock(stock_symbol, consolidated_symbols):
    # 從 consolidated_symbols (字典：group -> list of stock codes) 中找出 stock_symbol 所屬的族群
    for group, stocks in consolidated_symbols.items():
        if stock_symbol in stocks:
            return group
    return None

def update_group_observation_for_limit_up(stock_symbol, current_time, current_data, consolidated_symbols, wait_minutes, message_log):
    # 當漲停條件觸發：current_data中 high == 漲停價
    group = get_group_for_stock(stock_symbol, consolidated_symbols)
    if group is None:
        message_log.append((current_time.strftime('%H:%M:%S'), f"警告：股票 {stock_symbol} 未在族群中"))
        return
    # 如果該群尚未觀察或先前非觀察狀態，則設定觀察狀態（trigger: "漲停"）
    if group not in group_observation or group_observation[group].get("status") == "解除":
        group_observation[group] = {
            "status": "觀察中",
            "trigger": "漲停",
            "start_time": current_time,
            "tracking": {},   # 用以記錄該群中滿足進場條件（5min_pct_increase>=1.5）的股票
            "leader": None,
            "threshold": None,
            "waiting": False,
            "waiting_time": 0
        }
        message_log.append((current_time.strftime('%H:%M:%S'), f"股票 {stock_symbol} 漲停觸發，群組 {group} 進入觀察狀態(漲停)"))
    # 同時更新該群追蹤清單：只記錄 5min_pct_increase >= 1.5 的股票
    if current_data.get("5min_pct_increase", 0) >= 1.5:
        group_observation[group]["tracking"][stock_symbol] = current_data.get("rise", 0)
    # 優先級：漲停條件較高

def update_group_observation_for_pull_up(stock_symbol, current_time, current_data, consolidated_symbols, message_log):
    # 當拉高條件觸發：5min_pct_increase >= 2.0（且未漲停）
    group = get_group_for_stock(stock_symbol, consolidated_symbols)
    if group is None:
        message_log.append((current_time.strftime('%H:%M:%S'), f"警告：股票 {stock_symbol} 未在族群中"))
        return
    # 如果該群尚未觀察，則建立觀察狀態（trigger: "拉高"）
    if group not in group_observation or group_observation[group].get("status") == "解除":
        group_observation[group] = {
            "status": "觀察中",
            "trigger": "拉高",
        ...
        message_log.append((current_time.strftime('%H:%M:%S'), f"股票 {stock_symbol} 拉高觸發，群組 {group} 進入觀察狀態(拉高)"))
    # 更新追蹤清單：只記錄 5min_pct_increase >= 1.5 的股票
    if current_data.get("5min_pct_increase", 0) >= 1.5:
        group_observation[group]["tracking"][stock_symbol] = current_data.get("rise", 0)

def process_group_observations(current_time, wait_minutes, consolidated_symbols, auto_intraday_data, message_log):
    """
    檢查每個處於觀察中的群組，若等待期達到則選股進場（根據不同 trigger 分別處理）。
    """
    entered_groups = []  # 儲存已進場的群組
    for group, obs in list(group_observation.items()):
        # 若該群已被鎖定（已進場），則跳過
        if obs.get("status") == "已進場":
            continue
        elapsed = (current_time - obs["start_time"]).total_seconds() / 60.0
        if elapsed < wait_minutes:
            # 尚未達到等待期，更新等待時間
            obs["waiting_time"] = int(elapsed)
            message_log.append((current_time.strftime('%H:%M:%S'), f"群組 {group} 仍在等待中（{obs['trigger']}），等待 {wait_minutes - obs['waiting_time']} 分鐘"))
            # 同時，若該群 trigger 為「拉高」，則更新領漲：找追蹤清單中 rise 最大者
            if obs["trigger"] == "拉高" and obs["tracking"]:
                leader = max(obs["tracking"].items(), key=lambda x: x[1])[0]
                if leader != obs["leader"]:
                    obs["leader"] = leader
                    message_log.append((current_time.strftime('%H:%M:%S'), f"群組 {group} 更新領漲：{leader}，rise：{obs['tracking'][leader]:.2f}%"))
            continue
        # 已達到等待期，進行進場選股：
        # 從觀察中該群的追蹤清單中，排除觸發進場（漲停或拉高）所產生的股票（例如：漲停時不進場該股票本身）
        tracking = obs["tracking"]
        if not tracking:
            # 如果追蹤清單為空，則解除觀察
            message_log.append((current_time.strftime('%H:%M:%S'), f"群組 {group} 等待期結束但無有效追蹤股票，解除觀察"))
            group_observation[group]["status"] = "解除"
            continue
        # 取當前每隻股票最新的 rise 值（這邊假設 auto_intraday_data 中的最後一根資料代表最新）
        current_rise_values = {}
        for stock in tracking.keys():
            data_list = auto_intraday_data.get(stock, [])
            if data_list:
                latest_data = data_list[-1]
                current_rise_values[stock] = latest_data.get("rise", 0)
            else:
                current_rise_values[stock] = 0
        # 排序（降序），然後選取中間偏後的股票（即中位數後的那支）
        sorted_stocks = sorted(current_rise_values.items(), key=lambda x: x[1], reverse=True)
        if len(sorted_stocks) <= 1:
            # 若只有一支股票，則無法排除領漲股票；此處直接選取該股票
            entry_stock = sorted_stocks[0][0]
        else:
            # 排除領漲股票（假如領漲存在）
            if obs["leader"] in current_rise_values and len(sorted_stocks) > 2:
                candidates = [item for item in sorted_stocks if item[0] != obs["leader"]]
                # 選取中間偏後者：例如，取排序結果中索引為 len(candidates)//2 + 1 的股票
                idx_sel = min(len(candidates) - 1, (len(candidates) // 2) + 1)
                entry_stock = candidates[idx_sel][0]
            else:
                idx_sel = len(sorted_stocks) // 2
                entry_stock = sorted_stocks[idx_sel][0]
        message_log.append((current_time.strftime('%H:%M:%S'), f"群組 {group} 等待期結束，已進場股票：{entry_stock}"))
        # 呼叫進場函數進場（此處調用 entry_trade 函數；進場後將該群鎖定）
        entry_trade([{"symbol": entry_stock, "rise": current_rise_values[entry_stock]}],
                    current_time, current_time.strftime('%H:%M:%S'),
                    {}, 0,  # stock_data_collection 此處由 entry_trade 自行處理（或您可根據原程式呼叫）
                    message_log, [], {}, verbose=True)
        # 將該群狀態標記為已進場（鎖定）
        group_observation[group]["status"] = "已進場"
        entered_groups.append(group)
    # 對於已進場的群組，可從追蹤清單中清除觀察記錄（待退場後解除鎖定）
    for group in entered_groups:
        # 此處可加入鎖定後後續處理（例如：清空該群追蹤清單）
        group_observation[group]["tracking"].clear()
...
def process_live_trading_logic(
    ...
    盤中進場邏輯（進場條件分為【漲停進場】與【拉高進場】）：
      ...
      3. 若族群觸發【漲停進場】，每分鐘更新該族群追蹤清單並廣播等待狀態，
         待等待時間滿後從追蹤清單中選擇中間偏後的股票進場，並廣播進場結果。
         （修改後：廣播訊息格式為「漲停進場 <族群> 族群等待完成，進場 <股票代號>」改為用全新格式）
      4. 若族群觸發【拉高進場】，則：
           - 首次選定領漲股票時廣播（例如：「拉高進場 化學 族群，領漲：1711」）；
           - 當領漲股票反轉（即當前K線的 high <= 前一K線的 high）時廣播反轉並開始等待（例如：「拉高進場 化學 族群，領漲 1711 反轉，開始等待。」）；  
             此時將等待起始時間記錄下來，且等待時間的計數從該分鐘開始；
           - 在等待期間，每分鐘會直接以 print 輸出等待狀態訊息；
           - 在等待期間，如果追蹤清單中出現其他股票，其漲幅超過當前記錄的「領漲反轉漲幅」，則進行【領漲替換】，
             並廣播替換訊息（例如：「拉高進場 化學 族群，領漲替換，新領漲：1721」）；
        ...
        if trigger == "漲停進場":
            ...
            if tracking_list:
                ...
                if entry_price is None:
                    ...
                    # 根據 entry_trade 公式計算 shares
                    shares = round((capital_per_stock * 10000) / (entry_price * 1000))
                    sell_cost = shares * entry_price * 1000
                    entry_fee = int(sell_cost * (transaction_fee * 0.01) * (transaction_discount * 0.01))
                    tax = int(sell_cost * (trading_tax * 0.01))
                    msg = f"{GREEN}進場！股票代號：{chosen_stock['symbol']}，進場 {shares} 張。{RESET}"
                    message_log.append((current_time_str, msg))
                    print(msg)
                    in_position = True
                            ...
                            if grp_status.get("waiting_counter", 0) >= wait_minutes:
                                ...
                                if eligible_for_entry:
                                    ...
                                    if entry_price is None:
                                        ...
                                        entry_fee = int(sell_cost * (transaction_fee * 0.01) * (transaction_discount * 0.01))
                                        tax = int(sell_cost * (trading_tax * 0.01))
                                        entry_broadcast = f"{GREEN}進場！股票代號：{chosen_stock['symbol']}，進場 {shares} 張。{RESET}"
    ...
    # 最後可將結果寫回 auto_intraday.json 或進行後續處理
...
def update_intraday_json(auto_intraday_data, symbol, latest_candle):
    if symbol not in auto_intraday_data:
        auto_intraday_data[symbol] = []
    existing_candles = auto_intraday_data[symbol]
    
    # 如果成交量為 0，則需要補充數據
    if latest_candle.get('volume', 0) == 0:
        # 檢查該K線時間是否為 "09:00:00"
        if latest_candle.get('time') == "09:00:00":
            supplement_val = latest_candle.get("昨日收盤價")
        else:
            if existing_candles:
                supplement_val = existing_candles[-1].get("close", latest_candle.get("昨日收盤價"))
            else:
                supplement_val = latest_candle.get("昨日收盤價")
        latest_candle['open'] = supplement_val
        latest_candle['low'] = supplement_val
        latest_candle['high'] = supplement_val
        latest_candle['close'] = supplement_val
    
    # 將現有K線資料與最新K線合併後重新計算五分鐘漲幅和最高值
    ...
    # 對部分欄位進行截斷處理
    fields_to_truncate = ['漲停價', 'rise', '5min_pct_increase', 'highest']
    ...
    auto_intraday_data[symbol].append(calculated_candle)
    auto_intraday_data[symbol] = auto_intraday_data[symbol][-1000:]
